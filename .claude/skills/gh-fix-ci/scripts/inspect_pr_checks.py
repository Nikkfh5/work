#!/usr/bin/env python3
"""Inspect failing GitHub PR checks, fetch Actions logs, and extract failure snippets.
...
def run_gh_command(args: Sequence[str], cwd: Path) -> GhResult:
    process = subprocess.run(
        ["gh", *args], cwd=cwd, text=True, capture_output=True,
    )
...
def main() -> int:
    ...
    for check in failing:
        results.append(analyze_check(
            check, repo_root=repo_root,
            max_lines=max(1, args.max_lines), context=max(1, args.context),
        ))
...
def find_git_root(start: Path) -> Path | None:
    result = subprocess.run(
        ...
        cwd=start, text=True, capture_output=True,
    )
...
def fetch_checks(pr_value: str, repo_root: Path) -> list[dict[str, Any]] | None:
    ...
    result = run_gh_command(
        ["pr", "checks", pr_value, "--json", ",".join(primary_fields)], cwd=repo_root,
    )
    if result.returncode != 0:
        ...
        if available_fields:
            ...
            if not selected_fields:
                ...
                return None
            result = run_gh_command(
                ["pr", "checks", pr_value, "--json", ",".join(selected_fields)], cwd=repo_root,
            )
            if result.returncode != 0:
                message = (result.stderr or result.stdout or "").strip()
