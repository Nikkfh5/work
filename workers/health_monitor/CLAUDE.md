...
### mcp-server-git (для health audit)
- `git_log` — частота коммитов, автоматические задачи, активность репозиториев
- `git_status` — состояние репозиториев, незакоммиченные изменения

### mcp-server-fetch (для maintenance)
- Проверка доступности внешних сервисов
- Чтение документации инструментов при maintenance-задачах

### Context7
Обязательно для библиотек/фреймворков (уже настроен).
