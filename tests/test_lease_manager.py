...
from storage.db import init_db, create_task
...
def test_acquire_sets_status_to_running(db_path, task_id):
    ...
    with get_conn(db_path) as conn:
        row = conn.execute("SELECT status, locked_by FROM tasks WHERE id=?", (task_id,)).fetchone()
...
def test_acquire_increments_worker_attempt(db_path, task_id):
    ...
    with get_conn(db_path) as conn:
        initial = conn.execute("SELECT worker_attempt FROM tasks WHERE id=?", (task_id,)).fetchone()[0]
    ...
    with get_conn(db_path) as conn:
        row = conn.execute("SELECT worker_attempt FROM tasks WHERE id=?", (task_id,)).fetchone()
...
def test_second_acquire_returns_none(db_path, task_id):
    ...
    t1 = acquire_lease(task_id=task_id, worker_id="w1", db_path=db_path,
                        now_fn=make_now(), uuid_fn=make_uuid("tok-001"))
    t2 = acquire_lease(task_id=task_id, worker_id="w2", db_path=db_path,
                        now_fn=make_now(), uuid_fn=make_uuid("tok-002"))
    assert t1 == "tok-001"
    assert t2 is None
...
def test_stale_lease_can_be_acquired(db_path, task_id):
    ...
    acquire_lease(
        task_id=task_id, worker_id="w1", ttl=300, db_path=db_path,
    ...
    with get_conn(db_path) as conn:
        conn.execute(
            "UPDATE tasks SET locked_until='2020-01-01 00:00:00' WHERE id=?",
            (task_id,)
        )
    # Второй воркер захватывает истёкший lease
    token = acquire_lease(
        task_id=task_id, worker_id="w2", ttl=300, db_path=db_path,
        now_fn=make_now(),
...
def test_renew_lease_success(db_path, task_id):
    ...
    token = acquire_lease(
        task_id=task_id, worker_id="w1", db_path=db_path,
        uuid_fn=make_uuid("tok-001"),
    )
    result = renew_lease(task_id=task_id, worker_id="w1", lease_token="tok-001", db_path=db_path)
    assert result is True


def test_renew_wrong_token_returns_false(db_path, task_id):
    """Продление с неверным токеном возвращает False."""
    acquire_lease(task_id=task_id, worker_id="w1", db_path=db_path, uuid_fn=make_uuid("tok-001"))
    result = renew_lease(task_id=task_id, worker_id="w1", lease_token="wrong-token", db_path=db_path)
...
def test_renew_wrong_worker_returns_false(db_path, task_id):
    ...
    acquire_lease(task_id=task_id, worker_id="w1", db_path=db_path, uuid_fn=make_uuid("tok-001"))
    result = renew_lease(task_id=task_id, worker_id="w2", lease_token="tok-001", db_path=db_path)
    assert result is False
...
def test_release_lease_success(db_path, task_id):
    ...
    result = release_lease(
        task_id=task_id, worker_id="w1", lease_token="tok-001",
        new_status="done", db_path=db_path,
    )
    assert result is True
    with get_conn(db_path) as conn:
        row = conn.execute("SELECT status, locked_by, lease_token FROM tasks WHERE id=?", (task_id,)).fetchone()
...
def test_release_wrong_token_returns_false(db_path, task_id):
    ...
    acquire_lease(task_id=task_id, worker_id="w1", db_path=db_path, uuid_fn=make_uuid("tok-001"))
    result = release_lease(
        task_id=task_id, worker_id="w1", lease_token="wrong",
        new_status="done", db_path=db_path,
    )
    assert result is False
...
def test_release_stale_frees_expired_tasks(db_path, task_id):
    ...
    with get_conn(db_path) as conn:
        conn.execute("UPDATE tasks SET locked_until='2020-01-01 00:00:00' WHERE id=?", (task_id,))
    count = release_stale(db_path=db_path, now_fn=make_now())
    assert count == 1
    with get_conn(db_path) as conn:
        row = conn.execute("SELECT status, last_error_reason FROM tasks WHERE id=?", (task_id,)).fetchone()
